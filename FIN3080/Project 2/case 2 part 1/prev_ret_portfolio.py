# -*- coding: utf-8 -*-
"""
Created on Sun Apr 17 15:13:35 2022
...
ew_pf = df1.groupby(['allo','Trdmnt']).agg({'cum_ret':'mean'}).reset_index()
...
result = ew_pf.groupby('allo').agg({'ret':'mean'}).reset_index()
...
reg = reg[reg.ym >= '2007-02'].reset_index(drop = True)

reg['ym'] = pd.to_datetime(reg.ym).dt.to_period('M')

capm = pd.merge(reg, ew_pf, how = 'outer', on = 'ym')

capm = capm.drop(['cum_ret'],axis = 1)

capm = capm.sort_values(['ym','allo'])

capm = capm[(capm.allo == 0) | (capm.allo == 4)]

capm['sft_ret'] = capm.ret.shift(1)

capm['ls_ret'] = capm['sft_ret'] - capm['ret']
...
# plt.title('Long-Short portfolio CAPM alpha by chasing ups and downs over 2006-2022')
# x_major_locator=MultipleLocator(1)
# ax=plt.gca()
# ax.xaxis.set_major_locator(x_major_locator)
# plt.grid(axis = 'y')
