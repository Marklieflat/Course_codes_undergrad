...
proc sql; 
	...
	select a.*, (b.mktrf+b.rf) as mkt, b.mktrf, b.rf,b.smb, b.hml, b.umd
...
proc sql;
	...
	select distinct a.PERMNO, a.evtdate, date, a.ret-Intercept-b.mkt*a.mkt as ar 
	from evtrets(where=(evtwin_beg<=date<=evtwin_end)) as a
	inner join params(where=(_EDF_+_P_>&minest)) as b
...
proc sql;
	...
	select distinct day, mean(ar) as aar
	from  abrets_days
	group by day;
quit;
...
proc sql;
	...
	select distinct permno, evtdate, sum(ar) as car1 'CAR(-5,5)'
	from abrets_days 
	where -5<=day<=5
